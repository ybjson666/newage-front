...
UE.I18N['zh-cn'] = {
	...
	'wordimage': {
		'static': {
			...
			lang_step: "1、点击顶部复制按钮，将地址复制到剪贴板；2、点击添加照片按钮，在弹出的对话框中使用Ctrl+V粘贴地址；3、点击打开后选择图片,然后点击“开始上传”，最后点击底部的确定。"
		},
		'fileType': "图片",
...
function parseMmbizUrl(html) {
	...
	html = html.replace(/https:\/\/mmbiz.qlogo.cn/g, 'http://mmbiz.qpic.cn');
	return html;
}
...
function insertHtml(html, rules) {
	...
	var select_html = $.trim(getSelectionHtml(true));
	if (select_html != "") {
		if (rules) {
		...
		obj.find('> ._135editor').siblings('p').each(function(i) {
		...
		select_obj.find('img').each(function() {
			...
			if (img && img.size() > 0 && $(img).parents('.135brush').size() == 0) {
				...
				if (img.parent().attr('data-role') == 'circle' || img.parent().attr('data-role') == 'square' || img.parent().attr('data-role') == 'bgmirror') {
					img.parent().css('backgroundImage', 'url(' + $(this).attr('src') + ')');
				}
				img_i++;
				$(this).remove();
			}
...
function setBackgroundColor(bgcolor, color, history) {
	...
	if (!replace_full_color && active_item) {
		...
		$(current_editor.selection.document).find('._135editor').each(function() {
...
function parseObject(obj, bgcolor, color) {
	...
	obj.find("*").each(function() {
		...
		var oldColor = $(this).parents('._135editor:first').attr('data-color');
		var style = $(this).attr('style');
		if (oldColor == bgcolor) {
			return;
		}
		...
		if (bgimg.indexOf('linear-gradient(') >= 0) {
			...
			if (match) {
				...
				if ($(this).attr('data-grless')) {
					gradient_color = getColor(rgb2hex(main_color), $(this).attr('data-grless'), $(this).attr('data-grlessp'));
				} else if ($(this).attr('data-gradient')) {
				...
				for (var i = 0; i < match.length; i++) {
					...
					if ($(this).attr('data-grorder') == 'reverse') {
...
function clean_135helper() {
	var editor_document = current_editor.selection.document;
	$(editor_document).find('._135editor').each(function() {
...
function edit_image(url,callback) {
		xiuxiu.setLaunchVars("titleVisible", 0);
		xiuxiu.setLaunchVars("maxFinalWidth", 600);
		xiuxiu.setLaunchVars("uploadBtnLabel", "保存替换编辑器图片", "beautify");
		xiuxiu.embedSWF("MeituContent", 3, 1100,600, "beautify");
		xiuxiu.setUploadURL(window.editorData.dictionaryIsShowData.configs.editorUrl+'/editor/image/uploadMaterial?access_token='+window.access_token);
		xiuxiu.setUploadType(2);
		xiuxiu.setUploadDataFieldName("upload");
		console.log(window.editorData.dictionaryIsShowData.configs['050007']);
		...
		layer.open({
		  ...
		  area: [1100,600], //宽高
		  title: false, //不显示标题
		  content: $('#meituBox'), //捕获的元素，注意：最好该指定的元素要存放在body最外层，否则可能被其它的相对元素所影响
		  cancel: function(){
		  }
		});
		xiuxiu.onInit = function() {
				// 	console.log(UE);
				// console.log(UE.current_active_135item);
				// window.current_active_135item;
				// var aa = $(document.getElementById('ueditor_0').contentWindow);
				// console.log()
		...
		xiuxiu.onUploadResponse = function(res, id) {
			console.log(res);
			...
			if(res.state == 200){
				...
				if( $.isFunction(callback) ){
					...
					$(document.getElementById('ueditor_0').contentWindow.document.body).find(window.current_active_135item).attr('src',res.data[0].imageUrl+'?rand='+Math.random());
				}
			}else{
...
UE.plugins["open135"] = function() {
	...
	me.addListener('ready', function(editor) {
		...
		setTimeout(function() {
			$('<div></div>').load('./plug/ueditor/tpl.html', function() {
				var html = $(this).html();
				...
				obj.css('height', height).find('#style-overflow-list').height(height - 30);
				...
				$(document).on('click', '#style-categories .filter', function() {
					$('.editor-template-list > li', obj).fadeOut('slow', 'linear');
					$($(this).attr('data-filter'), obj).fadeIn('slow', 'linear');
				...
				$(obj).on('click', '#online-styles .col-sm-12,#style-overflow-list .ui-portlet-list > li,#styleSearchResultList #style_search_list > ul > li', function() {
					...
					var style_item = $(this).find('> ._135editor:first');
					if (style_item.size()) {
						...
						ret = insertHtml("<section data-id=\"" + id + "\" class=\"_135editor\">" + $(this).html() + "</section>");
					}
					if (ret) {
						style_click(id);
	...
	$.fn.extend({
		colorPicker: function(config) {
			var renderCallback = function(colors, mode) {
					...
					$input.val(colorMode === 'HEX' && !isAlpha ? '#' + (options.isIE8 ? AHEX : colors.HEX) : colorMode === 'rgb' || (colorMode === 'HEX' && isAlpha) ? (!isAlpha ? 'rgb(' + RGBInnerText + ')' : RGBAText) : ('hsl' + (isAlpha ? 'a(' : '(') + HSL.h + ', ' + HSL.s + '%, ' + HSL.l + '%' + (isAlpha ? ', ' + colors.alpha : '') + ')'));
				...
				doEventListeners = function(elm, multiple, off) {
					...
					$(elm)[onOff]('focus.colorPicker', function(e) {
						...
						if ($($input).parents('#color-plan').size() > 0) {
							left += $('#color-plan').offset().left - 260;
							top += $('#color-plan').offset().top + 20 + $(window).scrollTop();
