...
import { Breadcrumb, Button, Input, message, Modal, Popconfirm, Select, Table, Tooltip } from 'antd';
import './index.less';
        ...
        render: (tags, row) => {
          return (
            <div role="button" tabIndex="-1" style={{ whiteSpace: 'nowrap' }} onClick={(e) => {
              ...
              {row.state == 1 ? <Popconfirm placement="right" title={'此操作不可撤回,是否确认通过所选心愿申请?'} okType={'danger'}
                                            onConfirm={() => this.confirm([row], 2)}
                                            okText="确认" cancelText="取消">
                <a role="button" className='tableOperateButton' type="link">通过</a>
              </Popconfirm> : ''}
              {row.state == 1 ? <Popconfirm placement="right" title={'此操作不可撤回,是否确认驳回所选心愿申请?'} okType={'danger'}
              ...
              <a role="button" onClick={() => this.handleVisibleChange([row && row.wishid || ''], row)}
                 className='tableOperateButton' type="link">删除</a>
              {/*</Popconfirm>*/}
            </div>
          );
        }
