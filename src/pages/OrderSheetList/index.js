...
class TableList extends Component {
  ...
  onSelectChange = (selectedRowKeys, rows) => {
    ...
    rows.map(ele => {
      useRows.push(ele);
      useSelectedRowKeys.push(ele.activityorderid);
    });
  ...
  onRowEvent = row => {
    ...
    const selectRows = [];
    selectedRowKeys.push(row.activityorderid);
    selectRows.push(row);
...
// 获取列表
  getDataList = () => {
    ...
    resourceApi.getResourceList(listData).then(res => {
      ...
      this.setState({
        tableData: tableData,
        totalCount: res.data && res.data.data.totalCount || 0,
        isLoad:false
      },()=>{
  ...
  //查看详情
  lookDetail = (row) => {
    if (!row.activityorderid) {
      return;
    }
    const data = {
      id: row.activityorderid
    };
  ...
  mulpDelete = () => {
    ...
    if (!this.state.selectRows || this.state.selectRows.length < 1) {
      message.info('请选择需要删除的服务!');
      return;
    }
    const instance = this;
    confirm({
      title: '删除确认',
      content: '此操作不可撤回,是否确认删除所选点单服务?',
      onOk() {
  ...
  mulpDelay = () => {
    ...
    // });
    // if(status){
    //   message.error('当前所选项中包含已禁用的点单服务!');
    //   this.setState({
    //     selectRows: [],
    //     selectedRowKeys: []
    //   });
    ...
    const columns = [
      {
        ...
        render: (tags, row) => {
          ...
          return (<div>
            <div>{row.begintime?curtTime(row.begintime):'无限制'}</div>
            <div>{row.endtime&&curtTime(row.endtime)}</div>
          </div>);
        }
      },
        ...resourceForm,
        ...
        render: (tags, row) => {
          return (
            <div role="button" tabIndex="-1" style={{ whiteSpace: 'nowrap' }} onClick={(e) => {
              ...
              <a role="button" tabIndex="0" className='tableOperateButton' onClick={(e) => {
                this.confirm([row.activityorderid],row.enable);
              }}>{row.enable==1?'禁用':'启用'}</a>
              <Popconfirm placement="right" title={'此操作不可撤回,是否确认删除所选点单服务?'} okType={'danger'}
                          onConfirm={() => this.deleteItem([row.activityorderid])}
          ...
          <div style={{ height: '100%' }}>
            <div className="navTopBox">
              <div className='btnBox'>
                <Button type={'primary'} onClick={this.createActivity}>新建点单</Button>
              ...
              <div className='searchFatherBox searchLeftPadding'>
                <div className='searchBox '>
                 <span className='item-search-part'>
                    点单类型：<Select value={this.state.searchData.type} placeholder='全部' style={{ width: 120 }}
                                   onChange={(value) => {
                                     this.listData.pageNo = 1;
                ...
                <span className='item-search-part'>
                    点单状态：<Select value={this.state.searchData.state} placeholder='全部' style={{ width: 120 }}
                                   onChange={(value) => {
                                     this.listData.pageNo = 1;
                                     this.listData.pageSize = 10;
                  ...
                  <Option value="">全部</Option>
                  {this.state.serverState.map((ele, index) => {
                    return (<Option key={index} value={ele.dictid}>{ele.dictname}</Option>);
                  })}
                </Select>
                </span>
                  {this.pageStag=="B"? <span className='item-search-part'>
                服务单位：<Select value={this.state.searchData.unit} placeholder='全部' style={{ width: 120 }}
                             onChange={(value) => {
                               this.listData.pageNo = 1;
                               this.listData.pageSize = 10;
                               this.searchData.unit = value;
                               this.setState({
                                 searchData: this.searchData
                               });
                               this.getDataList();
