...
// 点单类型状态
import { orderTypeState as resourceTypeState } from '../../utils/utils';
...
// 获取列表
  ...
  mulpDealy = () => {
    ...
    instance.confirm(instance.state.selectedRowKeys,0);
    // confirm({
    //   title: '确认不通过',
  ...
  mulpAccept = () => {
    ...
    instance.confirm(instance.state.selectedRowKeys, 1);
    // confirm({
    //   title: '确认通过',
        ...
        render: (tags, row) => {
          return (
            <div role="button" tabIndex="-1" style={{ whiteSpace: 'nowrap' }} onClick={(e) => {
              ...
              {/*</Popconfirm>*/}
              <a role="button" tabIndex={0} className='tableOperateButton'   onClick={() => this.confirm([row.dictid],row.enable==1?0:1)} type="link">{row.enable==1?'禁用':'启用'}</a>
              <Popconfirm placement="right" title={'此操作不可撤回,是否删除所选点单类型标签?'} okType={'danger'}
        ...
        <div className='eachPageBox resourceTypeList-page'>
          <div style={{ height: '100%' }}>
            <div className="navTopBox">
              <div className='btnBox'>
                ...
                <Button type={'danger'} ghost onClick={this.mulpDealy}>禁用</Button>
          ...
          >
             ...
             <div className={'content-main'} >
               ...
               <div><Input onPressEnter={this.handleOk} className='form-input' maxLength={12} placeholder={'请在此输入点单类型名称'} value={this.state.typeName } onChange={(e)=>{
                 this.setState({
